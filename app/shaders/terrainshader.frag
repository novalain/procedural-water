...
void main() {
  float noise = 0.25  * snoise(vec3(0.05 * vUv.x, 0.05 * vUv.y, 1.0)) +
  ...
  vec4 terrainColor = mix(grass, sand ,0.7 * clamp(noise, 0.0, 1.0));

  // Interpolate between colors depending on elevation
  vec4 result = mix(bottomColor, terrainColor, clamp(elevation * 0.015, 0.0, 1.0));
